...
This challenge is a warmup challenge made to familiarize you with deploying your CCH23 project on Shuttle, and it does not count towards your score.
...
Every challenge is split into one or more _Core Tasks_ (marked with ⭐) and one or more _Bonus Tasks_ (marked with 🎁).

To **complete** a challenge, you only need to pass the tests for the _Core Tasks_ ⭐.

_Bonus Tasks_ 🎁 are harder to complete but give even more points. Their tests contain more edge cases and curveballs.

You can complete the _Core Tasks_ ⭐ first (submit at the bottom of the page), and then work on _Bonus Tasks_ 🎁. Your highest scoring submission is the one that counts.
...
The root endpoint `/` should respond with a `200 OK` status code to GET requests. Responding with a "Hello, world!" string, like the starter templates do, is enough to accomplish this.
...
Many tasks throughout CCH23 can easily be tested with a `curl` command. You can use it when testing locally, and then verify that the deployed app works.
...
You can at this point scroll to the bottom of this page to submit and see if your Hello World app is working, or keep going with the bonus task.
...
For this bonus task, add an endpoint on `/-1/error` that responds with the status code `500 Internal Server Error` to GET requests. The response body content does not matter.
